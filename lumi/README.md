...
- `inspect_harbor` + Modal fails during image build with missing `COPY` sources: this was caused by `inspect_sandboxes` dropping the Docker build context. See `../docs/modal-patch.md`; the local runtime patch should eventually be upstreamed to `inspect_sandboxes`.
