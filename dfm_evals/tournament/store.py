...
class TournamentStore(AbstractContextManager["TournamentStore"]):
    ...
    def initialize_from_config(self, config: TournamentConfig) -> None:
        ...
        with self.transaction():
            if len(active_model_ids) > 0:
                placeholders = ", ".join(["?"] * len(active_model_ids))
                conn.execute(
                    f"UPDATE models SET active = 0 WHERE model_id NOT IN ({placeholders})",
                    tuple(sorted(active_model_ids)),
                )
            else:
    ...
    def active_model_names_by_id(self) -> dict[str, str]:
        """Load active model names keyed by model_id."""
        rows = self.connection().execute(
            """
            SELECT model_id, model_name
            FROM models
    ...
    def match_status_counts_for_active_models(self) -> dict[str, int]:
        """Count matches grouped by status where both models are active."""
        conn = self.connection()
        rows = conn.execute(
            """
    ...
    def load_active_model_ratings(self) -> dict[str, ModelRating]:
        """Load active model ratings keyed by model_id."""
        conn = self.connection()
        rows = conn.execute(
            """
            SELECT r.model_id, r.mu, r.sigma, r.games, r.wins, r.losses, r.ties
            FROM ratings r
            JOIN models m ON m.model_id = r.model_id
            WHERE m.active = 1
            """
    ...
    def active_response_count(self) -> int:
        """Count responses belonging to active models."""
        conn = self.connection()
        row = conn.execute(
            """
